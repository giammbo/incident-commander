...
def connections_page(
    ...
    conns = list(db.scalars(select(SlackConnection).order_by(SlackConnection.id)))
    google = google_settings(db)
    google_conns = list(db.scalars(select(GoogleConnection).order_by(GoogleConnection.id)))
    return templates.TemplateResponse(
        request,
        "connections.html",
        {
            "current_user": user,
            "slack_connections": conns,
            "slack_configured": bool(slack.enabled and slack.client_id and slack.client_secret),
            "google": google,
            "google_connections": google_conns,
            "google_configured": bool(google.enabled and google.client_id and google.client_secret),
